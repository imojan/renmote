...
<div class="relative" 
     x-data="{ 
         ...
         async fetchNotifications() {
             try {
                 let response = await fetch('{{ route('notifications.checkUnread') }}');
                 if (response.ok) {
    ...
    <div x-show="open"
         ...
         class="absolute right-0 mt-3 w-80 sm:w-96 origin-top-right z-50">
                               ...
                               :class="!notification.read_at ? 'bg-blue-50/50' : ''">
                                <div class="flex items-start gap-3">
                                    ...
                                    <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full"
                                         :class="!notification.read_at ? 'bg-blue-100 text-blue-600' : 'bg-slate-100 text-slate-500'">
                                        <i class="fa text-sm" :class="notification.data.icon ? 'fa-' + notification.data.icon : 'fa-bell'"></i>
                                    </div>
                                    
                                    {{-- Content --}}
                                    <div class="flex-1 min-w-0">
                                        <p class="text-sm font-semibold text-slate-800 mb-1 line-clamp-2" x-text="notification.data.title || 'Notifikasi Baru'"></p>
                                        <p class="text-xs text-slate-600 mb-2 line-clamp-2" x-text="notification.data.message || ''"></p>
                                        <p class="text-xs text-slate-400">
                                            <i class="fa fa-clock mr-1"></i><span x-text="notification.created_at_human"></span>
                                        </p>
                                    </div>
